...
# 既定は 1192.168.40.228。同一PCで検証する場合は SERVER_IP=127.0.0.1 を指定してください。
SERVER_IP = os.getenv("SERVER_IP", "192.168.40.228")
...
async def main():
    ...
    if input_backend == "sounddevice":
        # デバイス指定（self/other 別々に）。未指定時は "Magii" を優先（部分一致可）。
        dev_self = os.getenv("SD_INPUT_DEVICE_SELF") or os.getenv("SD_INPUT_DEVICE") or "Magii"
    ...
    mute = MuteController()
    if use_sounddevice:
        # 出力デバイス指定（未指定なら "Magii" → None の順でフォールバック）
        out_dev = os.getenv("SD_OUTPUT_DEVICE") or os.getenv("SD_INPUT_DEVICE_SELF") or "Magii"
        try:
